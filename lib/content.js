...
export function getBlogs() {
  ...
  const blogSlugs = files.map((filename) => filename.replace('.md', '').toLowerCase())
...
export function getBlogsMetadata() {
  ...
  const blogsMetadata = blogSlugs.map((slug) => getBlogBySlug(slug, { withContent: false }))
  return blogsMetadata
}

export function getBlogBySlug(slug, { withContent = true } = {}) {
